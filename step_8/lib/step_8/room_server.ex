defmodule Step_8.RoomServer do
  use GenServer
  ...
  def enter_room(room_name, process) do
    GenServer.cast(__MODULE__, {:add_process_to_room, {room_name, process}})
  end

  def distribute_msg(room_name, msg) do
    GenServer.call(__MODULE__, {:send_msg_to_room, {room_name, msg}})
  end
  ...
  def handle_call({:send_msg_to_room, {room_name, msg}}, _from, state) do
    {:reply, send_msg_to_room(state, room_name, msg), state}
  end
  ...
  def handle_cast({:add_process_to_room, {room_name, process}}, state) do
    {:noreply, add_room_connection(state, room_name, process)}
  end
  ...
  defp add_room_connection(state, room_name, process) do
    Map.update(state, room_name |> String.downcase, default_room_state, fn(room_data) ->
      Keyword.update(room_data, :connections, [], fn(value) ->
        [process | value]
      end)
    end)
  end

  defp send_msg_to_room(state, room_name, msg) do
    get_room_by_name(state, room_name)
    |> get_room_connections
    |> send_msg_to_connections(msg)
  end

  defp get_room_connections(room_data) do
    Keyword.get(room_data, :connections)
  end

  defp send_msg_to_connections([], _), do: :ok

  defp send_msg_to_connections([head|tail], msg) do
    send(head, {:send_msg, msg})

    send_msg_to_connections(tail, msg)
  end
