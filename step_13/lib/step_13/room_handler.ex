defmodule Step_13.RoomHandler do
  use Plug.Router
  require EEx
  ...
  def room_to_html(conn, room_name, room_data) do
    ...
    |> Plug.Conn.send_resp(200, build_html_body(conn, room_name, room_data))
  end

  defp build_html_body(conn, room_name, room_data) do
    # IO.puts "Room Data: #{inspect room_data}"
    EEx.eval_file("templates/room.html.ex", assigns: [room_name: room_name, room_description: get_room_description(room_data), password_set: get_password_set(room_data), current_user: get_current_user_name(conn), current_user_is_owner: current_user_is_room_owner?(conn, room_name)])
  end
  ...
  defp current_user_is_room_owner?(conn, room_name) do
    RoomServer.get_room_owner(room_name) == get_current_user_name(conn)
  end
